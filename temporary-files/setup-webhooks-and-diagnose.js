...
async function subscribeAppToWABA() {
    console.log('📡 Step 1: Subscribing App to WABA for webhooks...\n');
    ...
    try {
        ...
        console.log('✅ App subscribed to WABA successfully!');
        ...
        if (error.response?.data?.error?.message?.includes('already subscribed')) {
            console.log('   ℹ️ App was already subscribed');
            return true;
        }
        return false;
    }
...
function startWebhookServer() {
    ...
    app.listen(PORT, () => {
        ...
        console.log('\n   ⚠️ IMPORTANT: Configure this URL in Meta App Dashboard:');
...
async function checkTemplateStatus() {
    console.log('\n🔍 Step 3: Verifying template status...\n');
...
async function sendToFreshRecipient() {
    console.log('\n📨 Step 4: Sending to FRESH test recipient...\n');
    ...
    console.log(`   ⚠️ NOTE: Replace the phone number with a fresh test number!`);
    ...
    try {
        ...
        console.log('✅ Message sent to fresh recipient!');
        console.log(`   Message ID: ${response.data.messages[0].id}`);
        console.log('\n   🔍 NOW WATCH THE WEBHOOK OUTPUT ABOVE!');
...
async function createUtilityTemplate() {
    console.log('\n📝 Step 5: Creating UTILITY template with IMAGE header...\n');
    ...
    try {
        ...
        console.log('\n   ⏳ Wait for approval (usually faster for UTILITY)');
        ...
        console.error('❌ Failed to create UTILITY template:', error.response?.data || error.message);
        return null;
    }
}
 ...
 */
async function main() {
    console.log('🚀 WEBHOOK SETUP & DIAGNOSTIC TESTS');
    console.log('=' .repeat(60));
    ...
    console.log('\n' + '=' .repeat(60));
    console.log('🧪 DIAGNOSTIC TEST 1: Fresh Recipient');
    console.log('=' .repeat(60));
    ...
    if (template && template.status === 'APPROVED') {
        ...
        if (messageId) {
            console.log('\n⏳ Waiting 30 seconds for webhook response...');
    ...
    console.log('\n' + '=' .repeat(60));
    console.log('🧪 DIAGNOSTIC TEST 2: UTILITY Template');
    console.log('=' .repeat(60));
    ...
    console.log('\n' + '=' .repeat(60));
    console.log('📊 WEBHOOK SERVER RUNNING - KEEP THIS OPEN!');
    console.log('=' .repeat(60));
