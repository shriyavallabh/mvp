...
const templates = [
    ...
    },
    {
        ...
        example: ['Avalok', '₹5 Crore', '60', '₹35,000'],
        category: 'UTILITY'
    },
    {
        ...
        example: ['Shruti', '₹50 Lakhs', '₹25,000'],
        category: 'UTILITY'
    }
];

async function createUploadSession(fileSize, fileName) {
    console.log(`📤 Creating upload session for ${fileName}...`);
...
async function uploadImage(uploadSessionId, imagePath, fileSize) {
    ...
    try {
        ...
        console.log(`✅ Image uploaded: ${response.data.h}`);
...
async function createTemplate(templateConfig, headerHandle) {
    ...
    try {
        ...
        if (error.response?.data?.error?.message?.includes('already exists')) {
            console.log(`ℹ️ Template ${templateConfig.name} already exists`);
            return { exists: true };
        }
...
async function checkTemplateStatus(templateName) {
    console.log(`🔍 Checking status for template: ${templateName}...`);
...
async function sendTestMessage(templateName, parameters) {
    console.log(`📨 Sending test message with template: ${templateName}...`);
...
async function processTemplate(templateConfig) {
    ...
    try {
        ...
        if (existingTemplate && existingTemplate.status === 'APPROVED') {
            console.log('✅ Template already approved! Sending test message...');
        ...
        if (!templateResult.exists) {
            console.log('⏳ Template submitted for approval. This may take 1-24 hours.');
        ...
        console.error(`❌ Failed to process template ${templateConfig.name}:`, error.message);
    }
}

async function main() {
    console.log('🚀 WhatsApp Image Template Workflow');
    console.log('====================================\n');
    ...
    console.log('⏳ Templates pending approval will be available in 1-24 hours');
    console.log('📊 Check template-tracking.json for status tracking');
    console.log('\n💡 Next steps:');
